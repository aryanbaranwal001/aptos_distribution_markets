...
module distribution_markets::distribution_markets {
    ...
    // Lending helpers
    fun lender_supply_with_signer(_caller: &signer, _amount: u64, _asset_metadata: Object<Metadata>) {
        let _ = _caller; let _ = _amount; let _ = _asset_metadata;
    }
    fun lender_withdraw_all_with_signer(_caller: &signer, _asset_metadata: Object<Metadata>) {
        let _ = _caller; let _ = _asset_metadata;
    }
    ...
    struct Market has key {
        ...
        /// Total principal contributed (sum of all deposits considered for yield sharing)
        total_principal_contributed: u64,
        /// Principal contributed per address (for simple pro-rata yield split)
        principal_by_addr: Table<address, u64>,
        /// Total yield available to distribute at/after resolution
        total_yield_pool: u64,
        /// Whether an address has already claimed their yield share
        yield_claimed: Table<address, bool>,
    }
    ...
    public fun initialize_market(
        ...
        // Supply to lender using treasury signer
        let t_signer_init = account::create_signer_with_capability(&treasury_cap);
        lender_supply_with_signer(&t_signer_init, initial_b, collateral_metadata);
        ...
        let market = Market {
            ...
            treasury_cap,
            collateral_metadata,
            total_principal_contributed: 0,
            principal_by_addr: table::new(),
            total_yield_pool: 0,
            yield_claimed: table::new(),
        };
        ...
        // Record initial backing as LP principal contribution for yield
        let market_ref = borrow_global_mut<Market>(market_addr);
        record_contribution(&mut market_ref.principal_by_addr, &mut market_ref.total_principal_contributed, initial_lp, initial_b);
    ...
    fun calculate_settlement_payout(position: &Position, market: &Market): u64 {
        ...
        (settlement_u128 / 10000000000) as u64
    }
    ...
    public fun trade(
        ...
        // Supply new collateral to lender
        let t_signer_trade = account::create_signer_with_capability(&market.treasury_cap);
        lender_supply_with_signer(&t_signer_trade, cost, market.collateral_metadata);

        // Track principal contribution for yield split
        record_contribution(&mut market.principal_by_addr, &mut market.total_principal_contributed, trader_addr, cost);
    ...
    public fun close_position(
        ...
        let market = borrow_global_mut<Market>(market_addr);
        // Disallow closing before resolution
        ...
        // Transfer settlement from treasury to trader (principal + pnl)
        ...
        // Pay yield share if not already claimed
        claim_and_pay_yield_if_any(&t_signer, trader_addr, market);
    ...
    public fun add_liquidity(
        ...
        let tstore = primary_fungible_store::ensure_primary_store_exists<Metadata>(market.treasury_addr, market.collateral_metadata);
        fungible_asset::deposit(tstore, collateral);
        // Supply LP collateral to lender
        let t_signer_lp = account::create_signer_with_capability(&market.treasury_cap);
        lender_supply_with_signer(&t_signer_lp, yb_u64, market.collateral_metadata);

        // Track LP principal contribution for yield split
        record_contribution(&mut market.principal_by_addr, &mut market.total_principal_contributed, lp_addr, yb_u64);
    ...
    public fun remove_liquidity(
        ...
        let market = borrow_global_mut<Market>(market_addr);
        // Disallow removing liquidity before resolution to keep lending/yield simple
        assert!(market.state.is_resolved, EMARKET_NOT_RESOLVED);
        let lp_addr = signer::address_of(lp);
        ...
        // Transfer from treasury to LP (principal share)
        let tstore_from = primary_fungible_store::ensure_primary_store_exists<Metadata>(market.treasury_addr, market.collateral_metadata);
        ...
        // Pay yield share if not already claimed
        claim_and_pay_yield_if_any(&t_signer, lp_addr, market);
    ...
    public fun resolve(
        ...
        // Pull back all liquidity from lender into treasury before settling
        let t_signer_w = account::create_signer_with_capability(&market.treasury_cap);
        lender_withdraw_all_with_signer(&t_signer_w, market.collateral_metadata);
        ...
        // Compute yield pool now that all funds are back in treasury
        // Simple model: yield = total_collateral - total_principal_contributed
        if (market.total_collateral >= market.total_principal_contributed) {
            market.total_yield_pool = market.total_collateral - market.total_principal_contributed;
        } else {
            market.total_yield_pool = 0;
        };
    ...
    /// Record a principal contribution for an address
    fun record_contribution(
        principal_by_addr: &mut Table<address, u64>,
        total_principal_contributed: &mut u64,
        who: address,
        amount: u64,
    ) {
        if (amount == 0) return;
        if (table::contains(principal_by_addr, who)) {
            let current = table::borrow_mut(principal_by_addr, who);
            *current = *current + amount;
        } else {
            table::add(principal_by_addr, who, amount);
        };
        *total_principal_contributed = *total_principal_contributed + amount;
    }

    /// Pay yield share to an address if not yet claimed
    fun claim_and_pay_yield_if_any(
        treasury_signer: &signer,
        who: address,
        market: &mut Market,
    ) {
        // Only after resolution
        if (!market.state.is_resolved) return;
        // If no yield, skip
        if (market.total_yield_pool == 0) return;
        // Already claimed?
        if (table::contains(&market.yield_claimed, who)) {
            let already = table::borrow(&market.yield_claimed, who);
            if (*already) return;
        };
        // Need principal amount
        if (!table::contains(&market.principal_by_addr, who)) return;
        let principal = table::borrow(&market.principal_by_addr, who);
        if (*principal == 0) return;
        let total_p = market.total_principal_contributed;
        if (total_p == 0) return;
        // pro-rata = floor(total_yield_pool * principal / total_principal_contributed)
        let share = (market.total_yield_pool * *principal) / total_p;
        if (share == 0) {
            // Mark claimed to avoid repeated tiny transfers
            if (table::contains(&market.yield_claimed, who)) {
                let flag = table::borrow_mut(&mut market.yield_claimed, who);
                *flag = true;
            } else {
                table::add(&mut market.yield_claimed, who, true);
            };
            return;
        };
        // Transfer yield share
        let tstore_from = primary_fungible_store::ensure_primary_store_exists<Metadata>(market.treasury_addr, market.collateral_metadata);
        let tstore_to = primary_fungible_store::ensure_primary_store_exists<Metadata>(who, market.collateral_metadata);
        fungible_asset::transfer(treasury_signer, tstore_from, tstore_to, share);
        // Accounting
        if (market.total_yield_pool >= share) {
            market.total_yield_pool = market.total_yield_pool - share;
            market.total_collateral = market.total_collateral - share;
        } else {
            market.total_yield_pool = 0;
        };
        // Mark claimed
        if (table::contains(&market.yield_claimed, who)) {
            let flag2 = table::borrow_mut(&mut market.yield_claimed, who);
            *flag2 = true;
        } else {
            table::add(&mut market.yield_claimed, who, true);
        };
